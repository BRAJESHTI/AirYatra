...
@router.get("/public/airports")
async def get_public_airports(
    state: Optional[str] = None,
    limit: int = 50
):
    """Get list of airports (public - no auth required)"""
    db = get_database()
    ...
    airports = await db.landing_points.find(
        query,
        {"_id": 0, "id": 1, "name": 1, "code": 1, "city": 1, "state": 1, "icao_code": 1, "latitude": 1, "longitude": 1}
    ).limit(limit).to_list(limit)
...
@router.get("/public/helipads")
async def get_public_helipads(
    state: Optional[str] = None,
    type: Optional[str] = None,
    limit: int = 50
):
    """Get list of helipads (public - no auth required)"""
    db = get_database()
    
    query = {"type": {"$in": ["govt_helipad", "private_helipad"]}, "is_active": True}
    if state:
        query["state"] = {"$regex": state, "$options": "i"}
    if type:
        query["type"] = type
    ...
    helipads = await db.landing_points.find(
        query,
        {"_id": 0, "id": 1, "name": 1, "code": 1, "type": 1, "city": 1, "state": 1, 
         "category": 1, "latitude": 1, "longitude": 1, "permission_required": 1, "rent_applicable": 1}
    ).limit(limit).to_list(limit)
